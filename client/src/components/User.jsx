...
export const User = () => {
  ...
  useEffect(() => {
    axios.get('https://mern-deploy-server-7gsy.onrender.com/get')
      .then((result) => {
        // Ensure result.data is always an array
        if (Array.isArray(result.data)) {
          setUsers(result.data);
        } else {
          setUsers([]); // Set empty array if data is not in array format
        }
      })
      .catch(err => {
        console.log(err);
  ...
  const handleLogout = () => {
    axios.get('https://mern-deploy-server-7gsy.onrender.com/logout')
      .then(res => {
        if (res.data.status) {
          navigate('/');
        }
  ...
  const handleDelete = (id) => {
    axios.delete(`https://mern-deploy-server-7gsy.onrender.com/deleteUser/${id}`)
      .then(res => {
        console.log(res);
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full table-auto">
            ...
            <tbody className="text-gray-900 text-md font-semibold">
              {Array.isArray(users) && users.filter((item) => {
                ...
                <tr className="border-b border-gray-200 hover:bg-gray-100" key={user._id}>
                  ...
                  <td className="py-3 px-6 text-left">
                    ...
                    <button className="bg-red-500 text-white py-1 px-3 rounded text-sm hover:bg-red-600" onClick={() => handleDelete(user._id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
